    ...
    .then((data)=>{
        ...
        data.forEach(contest => {
            ...
            if(Running_Table.rows.length <= 1){
                document.getElementById('Running_Contest').innerHTML = `<p class="col-12 fs-2 mt-2" style="float:center">No running contests</p>`;
            }
            if(Upcoming_Table.rows.length <= 1){
                document.getElementById('Upcoming_Contest').innerHTML = `<p class="col-12 fs-2 mt-2" style="float:center">No upcoming contests</p>`;
            }
    })
    .catch((err)=>{
        console.log(err);
